...
const Chat: React.FC = () => {
    ...
    const fetchAssistantInstructions = async () => {
      try {
		...
		if (axios.isAxiosError(error)) {
		  console.error("Error fetching assistant configuration:", error.response?.data || error.message);
		} else if (error instanceof Error) {
		  console.error("Error fetching assistant configuration:", error.message);
		} else {
		  console.error("An unknown error occurred while fetching assistant configuration:", error);
		}
	}
    };
  ...
  const sendMessageApi = async (message: string) => {
    if (!apiKey || !assistantInstructions) {
      console.error("API Key or Assistant Instructions are not available.");
      return;
    }
